...
Now let's explore two windowing enhancements introduced in SQL Server 2022: the WINDOW clause, and the IGNORE NULLS clause added to the FIRST_VALUE and LAST_VALUE window functions.

## The WINDOW Clause

Let's start with the new WINDOW clause. First, let's create some sample data.
...
To address this issue, SQL Server 2022 introduces the `IGNORE NULLS` syntax for these functions:
...
Also note the tradeoff; without `IGNORE NULLS`, these values may return NULL for the truly lowest and highest stock dates. Using `IGNORE NULLS`, these values will never return NULL, but they will also ignore the lowest and highest stock dates in the event that they contain NULL.


Finally, delete the sample StockDate table we created for this demo:
