...
Start by pressing `CTRL+N` to open a new query window for this lab in SSMS. You'll be learning how to work with Row-Level Security over the next several labs, so when you complete this lab, keep the query window open before continuing to the next lab.
