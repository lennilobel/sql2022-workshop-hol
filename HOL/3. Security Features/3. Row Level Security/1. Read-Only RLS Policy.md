...
You'll be learning how to work with Row-Level Security over the next several labs, so when you complete this lab, keep the query window open before continuing to the next lab.
...
This sets the stage for our demo by creating a sample dataset. It's a straightforward scenario that will introduce the concept of "filter" predicates in RLS, aiming to grant read-only access to sales users for rows they own. In this case, the first three rows belong to SalesUser1, and the other three rows belong to SalesUser2.
...
[Lab: Row-Level Security (RLS) - Updateable Policy ▶](https://github.com/lennilobel/sql2022-workshop-hol/blob/main/HOL/3.%20Security%20Features/3.%20Row%20Level%20Security/2.%20Updateable%20RLS%20Policy%20on%20Shared%20Connection.md)
