...
Execute the following code to create the `InsertEmployee` stored procedure:
...
Finally, let's call the `InsertEmployee` stored procedure. Replace the `Main` method one last time, as follows:
