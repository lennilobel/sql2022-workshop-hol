...
In the Object Explorer, expand the Databases node, right-click on `LedgerLab`, and select "Delete". In the "Delete Object" dialog, select the "Close existing connections" checkbox. Then click "OK". This action will remove the `LedgerLab` database and all its data from your SQL Server instance.
